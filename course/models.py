...
class CourseOffer(models.Model):
    ...
    school_calendar = models.ForeignKey(SchoolCalendar, on_delete=models.CASCADE, default=1)  # Replace 1 with a valid ID
    available = models.BooleanField(default=True)
    
    def __str__(self):
        return f"{self.course.title} ({self.school_calendar.name})"
    
    def get_absolute_url(self):
        return reverse("courseoffer_detail", kwargs={"pk": self.pk})
